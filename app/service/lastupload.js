...
class LastuploadService extends Service {
  ...
  async updateLastUploadVideo(video) {
    ...
    const bookLastUpload = ctx.model.BookLastUpload.findOne({
      where: {
        book_id: {
          [Op.eq]: book.id
        }
    ...
    const chapterLastUpload = ctx.model.ChapterLastUpload.findOne({
      where: {
        chapter_id: {
          [Op.eq]: chapter.id
        }
    ...
    const sliceLastUpload = ctx.model.SliceLastUpload.findOne({
      where: {
        slice_id: {
          [Op.eq]: slice.id
        }
